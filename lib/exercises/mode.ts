...
import type { LearningMode, ModeBehaviour } from "@/types";

export const LEARNING_MODES = ["lesson", "class", "mock"] as const;
export const isLearningMode = (value: unknown): value is LearningMode => {
...
export const behaviourFor = (mode: LearningMode): ModeBehaviour => {
 ...
 */
export const modeForAttempt = (attempt: { examSessionId: string | null }): LearningMode => {
  return attempt.examSessionId ? "mock" : "class";
};

/** Whether feedback may be shown as soon as this attempt is submitted. */
export const revealsFeedbackOnSubmit = (mode: LearningMode): boolean => {
