...
import { isAutoCheckable } from "./course-constants";
import type {
  ChapterStatus,
  CourseChapter,
  ExerciseCheck,
...
export const lessonSubmitted = (result: LessonResult | undefined): boolean => {
  if (!result) return false;
  return (result.status ?? "COMPLETED") === "COMPLETED";
};

export const lessonPassed = (result: LessonResult | undefined): boolean => {
  if (!result) return false;
...
export const lessonInProgress = (result: LessonResult | undefined): boolean => {
  return !!result && !lessonPassed(result);
};

export const chapterLessonSlugs = (chapter: CourseChapter): string[] => {
  return chapter.topics.map((t) => t.lessonSlug);
};

export const chapterComplete = (chapter: CourseChapter, progress: ProgressMap): boolean => {
...
export const chapterStatus = (chapter: CourseChapter, progress: ProgressMap): ChapterStatus => {
 ...
 */
export const courseProgress = (progress: ProgressMap): {
  completed: number;
  total: number;
  ratio: number;
 ...
 */
export const resumePoint = (progress: ProgressMap): ResumePoint | null => {
 ...
 * fabricated verdict.
 */
export const gradeLessonExercise = (
