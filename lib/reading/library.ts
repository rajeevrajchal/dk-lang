import { plainText, wordCount } from "@/lib/learning/text";
import type {
  LibraryFilter,
  ReadingLevel,
  ReadingSummary,
...
export const estimatedMinutes = (entry: ReadingText): number => {
  return Math.max(1, Math.round(wordCount(entry.text) / WORDS_PER_MINUTE[entry.level]));
};

export const textLength = (entry: ReadingText): "short" | "medium" | "long" => {
...
export const toSummary = (entry: ReadingText): ReadingSummary => {
  return {
    id: entry.id,
    title: entry.title,
...
export const facetCounts = (entries: ReadingText[]): {
