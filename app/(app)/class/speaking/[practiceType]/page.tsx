...
//
// A numeric segment here is an old module URL — /class/speaking/2 used to mean
...
const Page = async ({ params }: { params: Promise<{ practiceType: string }> }) => {
  ...
  if (/^\d+$/.test(practiceType)) redirect("/class/speaking");

  return <TaskListPage category="SPEAKING" practiceTypeSlug={practiceType} />;
};
