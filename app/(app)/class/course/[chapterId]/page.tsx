...
import { lessonPassed, type ProgressMap } from "@/lib/curriculum/progress";
...
export default async function ChapterPage({
  ...
  const rows = await prisma.lessonProgress.findMany({ where: { userId: session!.user.id } });
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 sm:p-8 space-y-6">
      <Link href="/class/course" className="text-sm text-slate-500 hover:underline">
        {t.backToCourse}
      </Link>
      ...
      <ol className="rounded-xl border border-slate-200 bg-white divide-y divide-slate-100">
        {chapter.topics.map((topic, i) => {
          ...
          const done = lessonPassed(progress[topic.lessonSlug]);
          return (
            <li key={topic.id}>
              <Link
                href={`/class/course/${chapter.id}/${topic.lessonSlug}`}
